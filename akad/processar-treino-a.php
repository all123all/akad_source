...
<?php
/*Adicionar ID do professor que passou o treino junto com esse INSERT
ou inserir com um UPDATE, fica melhor que tentar adicionar no meio desses
vários ciclos 'for' */
...
Programar redirecionamento de página para evitar que a mesma informação
seja enviada para o BD duas vezes, de forma à ficar duplicada causando
...
echo $insert_aluno_treino_a; //Apagar essa linha futuramente
...
$insert_id_treino = 'SELECT treino_id FROM aluno_treino_a WHERE id_aluno = ' . $aluno_id;
$bd_treino_id_query = mysqli_query($link, $insert_id_treino);
$row = mysqli_fetch_assoc($bd_treino_id_query);
$bd_treino_id =  $row['treino_id'];

$update_aluno_id_treino = 'UPDATE aluno_dados SET id_treino_a = ' . $bd_treino_id . ' WHERE id = ' . $aluno_id;
