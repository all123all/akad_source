...
$serie_keep = array();
$obs_keep = array();
$insert_aluno_treino_b = 'INSERT INTO aluno_treino_b(';
...
$insert_aluno_treino_b .= 'id_aluno, ';

for ($i = 1; $i <= 15; $i++) {
    $insert_aluno_treino_b .= 'rep'.$i.', ';
}

for ($i = 1; $i <= 15; $i++) {
    $insert_aluno_treino_b .= 'rep'.$i.'_serie, ';
}

for ($i = 1; $i <= 15; $i++) {
    if($i < 15){
        $insert_aluno_treino_b .= 'rep'.$i.'_obs, ';
    }else{
        if($i == 15){
            $insert_aluno_treino_b .= 'rep'.$i.'_obs) VALUES (';
        }
    }

}

$insert_aluno_treino_b .= $aluno_id . ', ';

for ($i = 1; $i <= 15; $i++) {
    $insert_aluno_treino_b .= '"' . $exerc_keep[$i] . '"' . ', ';
}

for ($i = 1; $i <= 15; $i++) {
    $insert_aluno_treino_b .= '"' . $serie_keep[$i] . '"' . ', ';
}

for ($i = 1; $i <= 14; $i++) {
    $insert_aluno_treino_b .= '"' . $obs_keep[$i] . '"' . ', ';
}

$insert_aluno_treino_b .= '"' . $obs_keep[15] . '"' . ')';

echo $insert_aluno_treino_b; //Apagar essa linha futuramente
$result = mysqli_query($link, $insert_aluno_treino_b);

/*Essa query é responsável por jogar o valor de 'treino_id' de 
'aluno_treino' dentro de 'id_treino' em 'aluno_dados'*/

$insert_id_treino = 'SELECT id_treino FROM aluno_treino_b WHERE id_aluno = ' . $aluno_id;
$bd_treino_id_query = mysqli_query($link, $insert_id_treino);
$row = mysqli_fetch_assoc($bd_treino_id_query);
$bd_treino_id =  $row['id_treino'];

$update_aluno_id_treino = 'UPDATE aluno_dados SET id_treino_b = ' . $bd_treino_id . ' WHERE id = ' . $aluno_id;
